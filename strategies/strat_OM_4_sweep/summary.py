...
def calculate_metrics(df):
    ...
    side_col = 'visual_side' if 'visual_side' in df.columns else 'side'
    df['side_for_stats'] = (
        df[side_col]
        .astype(str)
        .str.strip()
    ...
    long_mask = df['side_for_stats'] == 'LONG'
    short_mask = df['side_for_stats'] == 'SHORT'
    long_count = int(long_mask.sum())
    short_count = int(short_mask.sum())
    long_profit = float(df.loc[long_mask, 'profit_dollars'].sum())
    short_profit = float(df.loc[short_mask, 'profit_dollars'].sum())
    profit_base = abs(long_profit) + abs(short_profit)
    if profit_base == 0:
        long_profit_pct = 0.0
        short_profit_pct = 0.0
    else:
        long_profit_pct = (long_profit / profit_base) * 100
        short_profit_pct = (short_profit / profit_base) * 100
...
def generate_html_report(metrics):
    ...
    long_short_payload = [
        {
            "side": "LONG",
            "profit_pct": round(metrics['long_profit_pct'], 2),
            "profit": round(metrics['long_profit'], 2),
            "count": metrics['long_count'],
        },
        {
            "side": "SHORT",
            "profit_pct": round(metrics['short_profit_pct'], 2),
            "profit": round(metrics['short_profit'], 2),
            "count": metrics['short_count'],
        },
    ]
    long_short_json = json.dumps(long_short_payload)
...
<div class="container">
            ...
            <h2 style="text-align: center; color: #666;">Trading Strategy Analysis</h2>

            <div class="chart-box">
                <h2>Long vs Short - % de Profit</h2>
                <div id="longShortChart"></div>
            </div>
            ...
            <div class="footer">
                Generated by Trading Strategy Backtest | {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}
            </div>
        </div>
        <script>
        (function() {{
            const lsData = {long_short_json};
            const sides = lsData.map(item => item.side);
            const profitPct = lsData.map(item => item.profit_pct);
            const hover = lsData.map(item => `${{item.side}}<br>Pct Profit: ${{item.profit_pct.toFixed(2)}}%<br>Profit: ${{item.profit.toLocaleString('en-US', {{minimumFractionDigits:2, maximumFractionDigits:2}})}}<br>Trades: ${{item.count}}`);
            const colors = sides.map(side => side === 'LONG' ? '#2ecc71' : '#e74c3c');

            const trace = {{
                x: sides,
                y: profitPct,
                type: 'bar',
                text: profitPct.map(v => `${{v.toFixed(2)}}%`),
                textposition: 'auto',
                marker: {{color: colors}},
                hoverinfo: 'text',
                hovertext: hover
            ...
            Plotly.newPlot('longShortChart', [trace], layout, {{displayModeBar: false}});
        }})();
        </script>
    </body>
    </html>
