...
SIGNAL_EMA_PERIOD = 0  # Periodo SMA/EMA para decidir el lado real de las señales
...
class OpenPosition:
    ...
    visual_side: Optional[str] = None  # Track lado derivado para reporting
...
class PendingOrder:
    ...
    # Market Profile signal data
    signal_data: dict = None  # Store all signal columns
    force_fill: bool = False  # Force entry on next tick regardless of price
    visual_side: Optional[str] = None  # Lado final usado para reportes/plots
...
def run_backtest_tickdriven(df_signals: pd.DataFrame, df_base: pd.DataFrame) -> pd.DataFrame:
    ...
    if SIGNAL_EMA_PERIOD and SIGNAL_EMA_PERIOD > 0:
        base["ema"] = (
            base["price"]
            .rolling(window=SIGNAL_EMA_PERIOD, min_periods=1)
            .mean()
        )
        ema_lookup = (
            base[["timestamp", "ema"]]
        ...
        sig = pd.merge_asof(
            ...
            on="timestamp",
            direction="backward",
        ).rename(columns={"ema": "ema_at_signal"})
    else:
        base["ema"] = np.nan
        sig["ema_at_signal"] = np.nan

    signals_list = sig.to_dict("records")
        ...
        for order in pending_orders:
            ...
            open_positions.append(
                OpenPosition(
                    ...
                    visual_side=order.visual_side or order.side,
                )
            )
    ...
    for i, row in base.iterrows():
        ...
        for pos in open_positions:
            ...
            if exit_reason:
                ...
                trade_record = {
                    ...
                    "side": pos.side,
                    "visual_side": pos.visual_side or pos.side,
        ...
        # 2. Check for new signals whose timestamp ya quedó detrás del tick actual
        while (
            next_signal_idx < total_signals
            and signals_list[next_signal_idx]["timestamp"] <= current_time
        ):
            signal_data = signals_list[next_signal_idx]
            signal_idx = signal_data['signal_idx']
            next_signal_idx += 1

            # Only block new signals when OPEN positions already hit the limit.
            if len(open_positions) >= NUM_MAX_OPEN_CONTRACTS:
                continue  # Pending entries are allowed but we skip creating new ones.

            raw_shape = signal_data.get('shape', 'mushroom_short')
            shape = str(raw_shape).strip().lower() or "mushroom_short"
            signal_price = float(signal_data['close_price'])
            ema_at_signal = signal_data.get("ema_at_signal")
            ...
            if shape == "d_shape":
                ...
                side = "SHORT"
                entry_price = signal_price + THRESHOLD_EXTRA
            elif shape in ("mushroom", "mushroom_short", "sweep_short", "mushroom_long", "sweep_long"):
                force_fill = True
                if (
                    SIGNAL_EMA_PERIOD
                    and SIGNAL_EMA_PERIOD > 0
                    and ema_at_signal is not None
                    and not pd.isna(ema_at_signal)
                ):
                    side = "LONG" if signal_price >= ema_at_signal else "SHORT"
                else:
                    side = "SHORT"
                visual_side = side
                entry_price = signal_price
            ...
            pending_orders.append(
                PendingOrder(
                    ...
                    visual_side=visual_side or side,
                )
            )
...
def main() -> pd.DataFrame:
    ...
    if "mushroom" in df_sig.columns:
        # Keep only rows explicitly flagged as Mushroom=True
        df_sig["mushroom"] = (
            df_sig["mushroom"]
            .astype(str)
            .str.strip()
        ...
        df_sig = df_sig[df_sig["mushroom"]]

    if "shape" in df_sig.columns:
        df_sig["shape"] = df_sig["shape"].str.strip().str.lower()
    else:
        df_sig["shape"] = "mushroom_short"
