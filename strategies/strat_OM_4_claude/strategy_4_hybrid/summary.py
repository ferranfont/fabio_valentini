...
def calculate_metrics(df):
    ...
    # Por categoría de señal (4 tipos específicos)
    p_above_green = df[df['signal_category'] == 'p_above_green']
    d_inside_green = df[df['signal_category'] == 'd_inside_green']
    d_below_red = df[df['signal_category'] == 'd_below_red']
    p_inside_red = df[df['signal_category'] == 'p_inside_red']
    ...
    return {
        ...
        # 4 categorías específicas
        'p_above_green_count': len(p_above_green),
        'p_above_green_profit': p_above_green['profit_dollars'].sum() if len(p_above_green) > 0 else 0,
        'd_inside_green_count': len(d_inside_green),
        'd_inside_green_profit': d_inside_green['profit_dollars'].sum() if len(d_inside_green) > 0 else 0,
        'd_below_red_count': len(d_below_red),
        'd_below_red_profit': d_below_red['profit_dollars'].sum() if len(d_below_red) > 0 else 0,
        'p_inside_red_count': len(p_inside_red),
        'p_inside_red_profit': p_inside_red['profit_dollars'].sum() if len(p_inside_red) > 0 else 0,
    ...
    <body>
        <div class="container">
            ...
            <table>
                ...
                <tr class="section-title">
                    <td colspan="2">SIGNAL BREAKDOWN (by Category)</td>
                </tr>
                <tr>
                    <td class="metric-label">p-Shape ABOVE Green (LONG)</td>
                    <td class="metric-value">{metrics['p_above_green_count']} ({metrics['p_above_green_count']/metrics['total_trades']*100:.1f}%)</td>
                </tr>
                <tr>
                    <td class="metric-label">p-Shape Above Green Profit</td>
                    <td class="metric-value {'positive' if metrics['p_above_green_profit'] > 0 else 'negative'}">${metrics['p_above_green_profit']:,.2f}</td>
                </tr>
                <tr>
                    <td class="metric-label">d-Shape INSIDE Green (LONG)</td>
                    <td class="metric-value">{metrics['d_inside_green_count']} ({metrics['d_inside_green_count']/metrics['total_trades']*100:.1f}%)</td>
                </tr>
                <tr>
                    <td class="metric-label">d-Shape Inside Green Profit</td>
                    <td class="metric-value {'positive' if metrics['d_inside_green_profit'] > 0 else 'negative'}">${metrics['d_inside_green_profit']:,.2f}</td>
                </tr>
                <tr>
                    <td class="metric-label">d-Shape BELOW Red (SHORT)</td>
                    <td class="metric-value">{metrics['d_below_red_count']} ({metrics['d_below_red_count']/metrics['total_trades']*100:.1f}%)</td>
                </tr>
                <tr>
                    <td class="metric-label">d-Shape Below Red Profit</td>
                    <td class="metric-value {'positive' if metrics['d_below_red_profit'] > 0 else 'negative'}">${metrics['d_below_red_profit']:,.2f}</td>
                </tr>
                <tr>
                    <td class="metric-label">p-Shape INSIDE Red (SHORT)</td>
                    <td class="metric-value">{metrics['p_inside_red_count']} ({metrics['p_inside_red_count']/metrics['total_trades']*100:.1f}%)</td>
                </tr>
                <tr>
                    <td class="metric-label">p-Shape Inside Red Profit</td>
                    <td class="metric-value {'positive' if metrics['p_inside_red_profit'] > 0 else 'negative'}">${metrics['p_inside_red_profit']:,.2f}</td>
                </tr>
            </table>
