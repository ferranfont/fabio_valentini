...
csv_path = "../data/ts_and_dom_2min.csv"
...
def plot_order_book(
    ax, dom_bid, dom_ask, current_price, profile_high=None, profile_low=None
):
    ...
    if profile_high is not None and profile_low is not None:
        ...
        all_dom_prices = [
            float(p) for p in set(list(dom_bid.keys()) + list(dom_ask.keys()))
        ]
        if not all_dom_prices:
            ax.text(
                0.5, 0.5, "No order book data", ha="center", va="center", fontsize=14
            )
            return
    ...
    max_size = max(
        max(bid_sizes) if bid_sizes else 1, max(ask_sizes) if ask_sizes else 1
    )
    ...
    if current_price and current_price in all_prices:
        ...
        ax.axhline(
            y=price_idx, color="blue", linewidth=2, linestyle="--", alpha=0.6, zorder=5
        )
        ax.plot(
            0,
            price_idx,
    ...
    stats_text += (
        f"BID/ASK: {total_bid_size/total_ask_size if total_ask_size > 0 else 0:.2f}"
    )
...
def plot_market_profile(ax, index):
    ...
    if not profile:
        ax.text(
            0.5, 0.5, "No data in rolling window", ha="center", va="center", fontsize=14
        )
    ...
    ax.set_title(
        f'Market Profile at {timestamp.strftime("%Y-%m-%d %H:%M:%S")}{close_str}\n'
...
def plot_merged(index):
    ...
    if not all_prices:
        ax.text(
            0.5, 0.5, "No price data in range", ha="center", va="center", fontsize=14
        )
        return
    ...
    ax.barh(
        y_positions,
        [-size for size in dom_bid_sizes],
        height=0.8,
        color=bid_color_dom,
        label="DOM BID",
        edgecolor="#666666",
    ...
    if closing_price and closing_price in all_prices:
        ...
        ax.axhline(
            y=price_idx, color="blue", linewidth=2, linestyle="--", alpha=0.6, zorder=25
        )
        ax.plot(
            0,
            price_idx,
    ...
    close_str = f" | Close: {closing_price:.2f}" if closing_price is not None else ""
    ax.set_title(
        f'Merged Order Book + Market Profile at {timestamp.strftime("%Y-%m-%d %H:%M:%S")}{close_str}\n'
        f"({PROFILE_WINDOW}s rolling window | Step {index+1}/{len(profiles_data)})",
        fontsize=11,
        fontweight="bold",
        pad=10,
    ...
    if profile:
        ...
        stats_text += (
            f"Profile BID: {total_mp_bid:.0f} | Profile ASK: {total_mp_ask:.0f}"
        )
