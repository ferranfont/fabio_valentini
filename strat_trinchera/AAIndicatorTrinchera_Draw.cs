...
namespace NinjaTrader.NinjaScript.Indicators
{
    public class AAIndicatorTrinchera_Draw : Indicator
    {
                    ...
                    if (parts[0].Trim().ToLower() == "orange_dot")
                    {
                        ...
                        if (parts.Length >= 5)
                        {
                            ...
                            try
                            {
                                ...
                                DrawOrangeDotSafe();
                                DrawLimitLinesSafe(sellLevel, buyLevel, startTime, endTime, signalCounter);
                            }
        ...
        // -----------------------------------------------------------------------------------------
        // DRAW LIMIT LINES: Draws horizontal lines for SELL and BUY limit levels
        // -----------------------------------------------------------------------------------------
        private void DrawLimitLinesSafe(double sellLevel, double buyLevel, DateTime startTime, DateTime endTime, int counter)
        {
            // Ensure ChartControl exists
            if (ChartControl == null || ChartControl.Dispatcher == null) return;
            ...
            ChartControl.Dispatcher.InvokeAsync(() =>
            {
                try
                {
                    ...
                    // Create unique tags for the lines
                    string sellTag = "SellLimit_" + counter;
                    string buyTag = "BuyLimit_" + counter;

                    // Draw RED horizontal line for SELL limit
                    // Draw.Line(this, tag, autoScale, startTime, startY, endTime, endY, brush, dashStyle, width)
                    Draw.Line(this, sellTag, false, startTime, sellLevel, endTime, sellLevel, Brushes.Red, DashStyleHelper.Solid, 1);

                    // Draw GREEN horizontal line for BUY limit
                    Draw.Line(this, buyTag, false, startTime, buyLevel, endTime, buyLevel, Brushes.LimeGreen, DashStyleHelper.Solid, 1);

                    // Debug print
                    Print(string.Format("[DRAW] Lines drawn - SELL: {0:F2} | BUY: {1:F2} | From {2} to {3}",
                        sellLevel, buyLevel, startTime.ToString("HH:mm:ss"), endTime.ToString("HH:mm:ss")));

                    ForceRefresh();
                }
                catch (Exception ex)
                {
                    Print(string.Format("[DRAW] Error drawing lines: {0}", ex.Message));
                }
            });
        }
