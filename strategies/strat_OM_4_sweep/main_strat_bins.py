...
def plot_trades():
    ...
    try:
        # Leer bins CSV (nueva estructura con peak_timestamp y peak_close_price)
        df_bins_raw = pd.read_csv(BINS_FILE, sep=';', decimal=',')

        # Crear DataFrame compatible renombrando columnas
        # USAR PEAK DATA (nueva lógica de entrada en el pico)
        df_signals_compatible = pd.DataFrame({
            'timestamp': pd.to_datetime(df_bins_raw['peak_timestamp']),  # Usar peak_timestamp (nueva lógica)
            'close_price': df_bins_raw['peak_close_price'].astype(str).str.replace(',', '.').astype(float),  # Usar peak_close_price
