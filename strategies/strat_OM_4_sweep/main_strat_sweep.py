"""
MAIN SCRIPT - Sweep Absortion Strategy
====================================

# Se usa Market Orders en vez de Limit Orders a efectos del backtest

Ejecuta la estrategia completa en orden:
1. Backtest con strat_absortion_shape.py (reutilizado para gestión de órdenes)
...
# PLASE HERE THE db_mushroom FILE YOU WANT TO USE
...
_signals_filename = f"db_mushroom_dom_{SIGNALS_DATE}.csv"
_signals_default = OUTPUTS_DIR / _signals_filename
_signals_absortion = OUTPUTS_DIR / "absortion_shape" / _signals_filename
_signals_sweep = OUTPUTS_DIR / "sweep" / _signals_filename
if _signals_default.exists():
    SIGNALS_FILE = _signals_default
elif _signals_sweep.exists():
    SIGNALS_FILE = _signals_sweep
elif _signals_absortion.exists():
    SIGNALS_FILE = _signals_absortion
else:
    raise FileNotFoundError(
        f"No se encontro el archivo de senales {_signals_filename} en {OUTPUTS_DIR}, {OUTPUTS_DIR / 'sweep'} ni {OUTPUTS_DIR / 'absortion_shape'}"
    )
# SIGNALS_FILE = OUTPUTS_DIR / "db_mushroom_dom_manual.csv"  # Alternativa: senales personalizadas
...
TRACKING_RECORD_FILE = _sweep_dir / f"sweep_absortion_TR_{SIGNALS_DATE}.csv"
TRADES_CHART_FILE = CHARTS_DIR / f"trades_visualization_sweep_absortion_{SIGNALS_DATE}.html"
BACKTEST_CHART_FILE = CHARTS_DIR / f"backtest_results_sweep_absortion_{SIGNALS_DATE}.html"
SUMMARY_REPORT_FILE = CHARTS_DIR / f"summary_report_sweep_absortion_{SIGNALS_DATE}.html"

# ========= PARÁMETROS DE ESTRATEGIA =========
SYMBOL = "NQ"
TP_POINTS = 12              # Take Profit en puntos
SL_POINTS = 10              # Stop Loss en puntos
POINT_VALUE = 20.0            # Valor del punto en dolares ($20 por punto para NQ)
THRESHOLD_EXTRA = 0              # margen de seguridad adicional
...
EMA_PERIOD = 152000                   # Periodo de la EMA a trazar en el chart
...
def run_backtest():
    ...
    strat_absortion_shape.SIGNAL_EMA_PERIOD = EMA_PERIOD
...
def plot_trades():
    ...
    plot_trades_chart.SIGNALS_FILE = str(SIGNALS_FILE)
...
def main():
    ...
    print_header("ESTRATEGIA SWEEP ABSORTION - EJECUCIÓN COMPLETA")
    ...
    try:
        ...
        # Paso 4: Resumen estadístico (incluye gráficos de resultados internamente)
        generate_summary()
