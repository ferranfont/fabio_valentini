"""
Transform NQ tick data (time_and_sales_nq.csv) into OrderFlowChart format.

This script reads tick-by-tick data and aggregates it into:
1. Orderflow data: bid_size, price, ask_size per price level per candle
2. OHLC data: open, high, low, close per candle
"""
...
CANDLE_INTERVAL = '1min'  # 1-minute candles (change to '5min' for 5-min, '15min' for 15-min, etc.)
MAX_CANDLES = 500  # Limit number of candles for visualization performance
...
# Use all data (comment out to limit for faster processing)
# if len(df) > 100000:
#     print(f"Sampling last 100,000 ticks for performance...")
#     df = df.tail(100000)
...
# ============================================================================
# 2. Create Orderflow data (volume by price level per candle)
# ============================================================================
...
print(f"Total volume: {orderflow_data['bid_size'].sum() + orderflow_data['ask_size'].sum():,}")

# Note: This script is no longer used. Use main.py which processes everything in memory.
